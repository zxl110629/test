function table_init(ajaxurl,aoColumns,aoColumnDefs,stateflag)//////tabel获取后台数据的初始化设置///////
{
	  $.ajaxSetup ({
     	 ...
     	 $("#tablelist").dataTable({
     	 	"oLanguage" : { // 汉化
						...
						"sInfo" : "当前显示_START_ 到 _END_ 条记录  总共 _TOTAL_ 条",
						"sInfoEmpty" : "记录数为0",
...
function xzqy_init(url)//////行政区域下拉列表获取后台数据的初始化设置///////
{	

	 $.ajax({                
			url : url+'qygl/xzqylist',             
			data : {},
			type : 'get',                
			dataType : 'json',                
			async : false,                
			success : function(items){ 
				jQuery.each(items, function(i,item){
					$("<option value='"+ item.split("|")[0] +"'>"+ item.split("|")[1] +"</option>").appendTo("#qy_op");
					});
			}                
				});
}

function xzqy_init1(url)//////行政区域下拉列表获取后台数据的初始化设置///////
{	
	 ...
	 $.ajax({                
			...
			success : function(items){ 
				jQuery.each(items, function(i,item){
					$("<option value='"+ item.split("|")[1] +"'>"+ item.split("|")[1] +"</option>").appendTo("#qy_op");
					});
			}                
				});
}
